...
class MergeMetrics(object):
    ...
    def __init__(self):
        ...
        self.solmetant_file = self.config['DEFAULT']['solmetant']
        self.overview_metrics = False

    def _get_addresses(self):
        '''
        get the addresses which are in contracts_overview_file but not in solmetant_file
        '''
        return list(
            set(self._get_addresses_from_contracts_overview()) - set(self._get_addresses_from_solmetant()))
    ...
    def _get_addresses_from_solmetant(self):
        with open(self.solmetant_file, 'r') as f:
